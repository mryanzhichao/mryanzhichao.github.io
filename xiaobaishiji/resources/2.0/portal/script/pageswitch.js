(function($){
	...
	$(document).on('touchstart',touchstart);
		function touchstart(e){
			//e.preventDefault();
			value_start =  e.originalEvent.changedTouches[0].clientY;
		};
	$(document).on('touchend',touchend);
		function touchend(e){
			//e.preventDefault();
			value_end = e.originalEvent.changedTouches[0].clientY;
	...
	function initPagination(){
		...
		var pageHtml = '<ul id="pages"><li class="active"><span class="q2"><span class="q3"></span></span></li>';
		for(var i=1;i<5;i++){
			pageHtml += '<li><span class="q2"><span class="q3"></span></span></li>';
		}
		pageHtml += '</ul>';
	...
	function reBuild(){
		...
		if(opts.direction == "horizontal"){
			...
			if((offsetTop)>currentHeight/2 && iIndex <(arrElement.length-1)){
				iIndex ++;
			}
		}
		if(iIndex){
